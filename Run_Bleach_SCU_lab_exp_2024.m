...
tspan = [0 23]; % Days (integer format)
...
photon2ros = 3500; % number of photons required to produce 1 ROS
chla2rcii = 0.002/893.49;
CSmaxbleachrate = 1.0; % d-1
repair_coefficient = 268; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% initial conditions are passed to bleach and then re-calculated at each timepoint:
...
% starting ROS is half the threshold (Suggett, 2008)
...
% experimental averaged nutrients:
...
SDate = datetime(2022,04,22,23,58,0); % Starting datetime [format = (Year,Month,Day,Hour,Minute,Second)]
...
% Some calculations
Aeff = 1 - exp(-OCH * y0(5));
% Aeff is the effective projected area fraction of the coral community

normalisedprojectedarea =  CS_N0 / mN * pi * rCS * rCS/Aeff; %m2
% normalised projected area of the symbiont
