...
% translocate (+ve symbiont to host), including growth translocates mortality of symbionts and mortality of host (-ve). 
...
% the units for dydt(4) have been corrected since this is in mmol photon cell-1 d-1
...
%Qi2Qox = 268 * photon2rcii * Qin * 86400; % this repairs 10 mol ph m-2 d-1 hitting 1 mmol of RCII.
...
if ROS <= ROSthreshold/2
    ARO=0;
elseif ROS > ROSthreshold/2 && ROS <= ROSthreshold
    ARO = uCS * Nquota * Iquota * Pquota * (ROS - ROSthreshold/2)*cells;
elseif ROS > ROSthreshold
...
dydt(12) = dydt(12) - ARO + (Qin/Qtot) * absorb / photon2rcii / photon2ros / 86400; % no loss of ROS from intracellular to extracellular

% now need to expel:

ROSpercell = y(12)/cells;

% Do we need a background expulsion rate?
% Background ROS expulsion is always occurring but this is small and perhaps insignificant
