...
% Mass transfer rate coefficient (height of water cleared of nutrient per unit of time by the watercoral 
% exchange): 

Sx_N = 2850 * (2 * tau / rho)^0.38 * Sc_N^-0.6;  % (m d-1)
Sx_P = 2850 * (2 * tau / rho)^0.38 * Sc_P^-0.6; % (m d-1)

Aeff = 1 - exp(-OCH * CH_N); % Aeff is the effective projected area fraction of the coral community (m2 m?2) 
...
if (CS_N * 5.6786 < C2Chlmin * Chl ) %  don't synthesise if can't fit in any more. */
  dChldt_syn = 0.0;
end
if Ed == 0
  dChldt_syn = 0.0;
...
Iuptake = Ed * aA * Qox/Qtot * a_rub * W2mmol_490 * (1.0 - Iquota) * cells; % mmol m-2 d-1
...
dydt(4) = dydt(4) + Iuptake - (symb_grow_IN * CS_N +  translocate_attNR * Iquota)* ((106/16) * (12.01/14.01)); % Redfield ratio 106C:16N:1P
									 
% xanothophyll switching
...
% 268 might have time units in it, and need to be divided by 86400?
									 
Qi2Qox = 268.0 * photon2rcii * Qin * 86400; % this repairs 10 mol ph m-2 d-1 hitting 1 mmol of RCII.
ARO = uCS * Nquota * Iquota * Pquota * ROS;  %  rate of detoxification.
...
dydt(12) = dydt(12) - ARO + 0.5 * (Qin/Qtot) * absorb / photon2rcii / photon2ros / 86400;   %ROS

% why do we have 0.5 in above equation? Only seems to work with /86400, which I don't understand
